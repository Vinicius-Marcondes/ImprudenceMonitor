<?php
	...
	$selecao_veiculo = "SELECT * FROM automoveis_empresa WHERE ID_AUTOMOVEIS = '$id'";
	...
	if(empty($_POST['placa']) || $_POST['placa'] == $veiculo['PLACA_AUTOMOVEIS']){
		...
		if($consulta_placa->num_rows == 0){
			...
			header("Location: ../editar_veiculosempresa.php?id=$id");
		}
	}
		
		$set_veiculo = "UPDATE automoveis_empresa SET NOME_AUTOMOVEIS = '$nome', MODELO_AUTOMOVEIS = '$modelo', MARCA_AUTOMOVEIS = '$marca', TIPO_AUTOMOVEIS = '$tipo', EQUIPAMENTOS_ID_AUTOMOVEIS = '$equipamento', PLACA_AUTOMOVEIS = '$placa', PESSOAS_ID_AUTOMOVEIS = '$pessoa_id' WHERE ID_AUTOMOVEIS = '$id'";
