                    ...
                    <div class="container">
						...
						<div class="panel panel-border panel-primary">
							<?php 
                                ...
                                $funcao = $pessoa['ID_PERFIL'];
							?>
                            <div class="panel-heading"> 
                                <h3 class="panel-title">Alterar função do usuário <?php echo "'".$pessoa['NOME_PESSOAS']."'"; ?></h3> 
                            </div> 
                            <div class="panel-body"> 
								<?php if(isset($_SESSION['msg_edit_usuario'])){ echo $_SESSION['msg_edit_usuario']; unset($_SESSION['msg_edit_usuario']); } if(1 == $_SESSION['perfil']){ ?>
                                <form role="form" method="post" action="processos/editar_funcao.php">
                                    
                                    <select name="funcao" required>
                                        <?php 
                                            $selecao_perfil = "SELECT * FROM perfil WHERE ID_PERFIL = $funcao";
                                        ...
                                        <?php } ?>
                                        <?php 
                                            $selecao_perfil = "SELECT * FROM perfil WHERE ID_PERFIL != 2 AND ID_PERFIL != $funcao";
                                            $consulta_perfil = $conexao_bd->query($selecao_perfil);
                                            while($perfil = mysqli_fetch_assoc($consulta_perfil)){
                                        ?>
                                        <option value="<?php echo $perfil['ID_PERFIL']; ?>"><?php echo $perfil['NOME_PERFIL']; ?></option>
									...
									<div class="row" style="margin-top: 30px;">
									   <div class="col-lg-11 col-md-9 col-sm-9 col-xs-5"><a href="usuarios.php">Voltar</a></div>
								...
								<?php } else { echo "Acesso Restrito. <a href='usuarios.php'>Voltar</a>"; } ?>
                            </div> 
						</div>
                    </div> 
                </div>
